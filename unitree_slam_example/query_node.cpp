...
int main(int argc, const char **argv)
{
    ...
    // Query all point information
    send_msg.seq_().data() = "index:123;";     // The value 123 is set by the user (1-10000).
    send_msg.command_() = 2;                   // 2 is a query instruction
    send_msg.attribute_() = 1;                 // 1 is select query node
    send_msg.floor_index_().push_back(999);    // Floor differentiation, please give a fixed value of 999
    send_msg.node_edge_name_().push_back(999); // The list of names of the queried nodes. When node_edge_name[0]=999, all edge information will be queried.

    publisher->Write(send_msg);
    std::cout << "send query node command" << std::endl;
    sleep(2);
