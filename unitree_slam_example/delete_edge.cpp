...
int main(int argc, const char **argv)
{
    ...
    send_msg.node_edge_name_().push_back(0); // The names of the edges to be deleted, supports multiple deletions simultaneously; when node_edge_name[0]=999, all edges will be deleted.

    publisher->Write(send_msg);
    std::cout << "send delete edge command" << std::endl;
    sleep(2);
